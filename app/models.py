...
class Transaction(db.Model):
    ...
    status = db.Column(db.Enum('complete', 'declined', 'pending', 'under review') nullable=False)
...
class Loan(db.Model):
    __tablename__ = 'loans'
    id = db.Column(uuid4().hex, primary_key=True, nullable=False)
    user_id = db.Column(db.ForeignKey('users.id'), nullable=False) # Remove this maybe
    borrower = db.Column(db.ForeignKey('users.id'), nullable=False) # look into this
    amount = db.Column(db.Float, nullable=False)
